...
```
上面程序的输出是：`10 10 0`。我们将同一个函数对象赋值给了两个function，然后分别调用了这两个function，但函数对象中nSum的值并没有被修改。这是因为function的默认行为是拷贝一份传递给它的函数对象，于是f1和f2中保存的都是counter对象的拷贝，调用f1和f2后，counter对象中的值并没有被修改。
